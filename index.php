...
{	
	add_menu_page('ECT Product Carousel','ECT Product Carousel','manage_options','ect_prod','ect_prod_fun',plugin_dir_url(__FILE__).'img/ect28x28.png',1500);	
...
function plugin_sett($links) { 
  $settings_link = '<a href="admin.php?page=ect_prod">Settings</a>'; 
  array_unshift($links, $settings_link); 
  return $links; 
}
 
$plugin = plugin_basename(__FILE__); 
add_filter("plugin_action_links_$plugin", 'plugin_sett' );
...
?>
	...
	<li>
		<input type="submit" accesskey="p" value="Save" class="btn button-primary button-large" id="publish" name="publish">
	</li>	
	</ul>	
	</form>
	<style>	
		...
		if(!empty($_POST))	
		{
			$pauseonhover=esc_sql($_POST['pauseonhover']);
			$enable_break=esc_sql($_POST['enable_break']);
			$autoplay=esc_sql($_POST['autoplay']);
			$opt=esc_sql($_POST['opt']);
			
			$v1=isset($pauseonhover) ? 1 : 0;$v2=isset($enable_break) ? 1 : 0;
			$v3=isset($autoplay) ? 1 : 0;
	...
	add_action('wp_footer','inst_foot');
	function inst_foot()
	{	
		wp_enqueue_style('ect-css_prod_caro1',plugin_dir_url(__FILE__).'css/style.css');	
		wp_enqueue_script('jquery');	
		wp_enqueue_script('ect-jq1_prod_caro1',plugin_dir_url(__FILE__).'js/jquery.flexisel.js');
	}
	function ect_prod_fun()
	{
	...
	{	
		...
		{	
			$StockMgr='';	
			$IsStockMgrEnable=$ECTWPDB->get_results('select adminStockManage from admin where adminID=1');	
			if(!empty($IsStockMgrEnable) && !empty($IsStockMgrEnable[0]->adminStockManage))	
				$StockMgr='and p1.pInStock >0';		
				$ID=$atts['id'];	
				...
				elseif($Dat['slide_images_from']=='recom')	
				{
					$cond=' and p1.pRecommend=1';		
				}	
				global $wpdb;
				$str='';
				$list_prod=get_option('ect_prod_sett_list_prod');
				$vi=get_option('ect_prod_sett_visible_prod');
				$Q=$ECTWPDB->get_results("select p1.pID,p1.pName,p1.pSection,p2.imageSrc from ".$db_name.".products as p1 inner join ".$db_name.".productimages as p2 on p1.pID=p2.imageProduct $inn where p2.imageType=0 and p1.pDisplay=1 $StockMgr $cond limit $list_prod ");
